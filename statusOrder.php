...
?>
<html>
    <title>Pesanan Dalam Proses</title>
    <head>
        <meta charset="UTF-8">
...
<center><h1>Status Pesanan</h1></center>
        ...
        <table class="table table-striped .table-borderless">
            <thead class="thead-dark">
                <tr>
                    ...
                    <th><center>Konfirmasi Pesanan</center></th>
                </tr>
            </thead>
    </center>
    <br>
<?php
    include './query/selectPaymentStatus.php';
    while($row = mysqli_fetch_array($result)) {
        ...
        echo('
            <tr>
                ...
                <td>
            ');
            if($row['status'] == 'Pembayaran telah diterima dan disetujui'){
                echo ('
                    <div class="dropdown">
                        <center>
                            <button class="btn-lg fas fa-check dropdown-toggle" type="button" data-toggle="dropdown">
                            <span class="caret"></span></button>
                                <ul class="dropdown-menu" style="padding: 10px; min-width: 100px; margin-left: 50px;">
                                    <form method="POST" name="transactionId" id="deleteForm">
                                        <center><p>Konfirmasi pesanan telah diterima ?</p></center>
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "UPDATE cart SET isReceived = 1 WHERE transactionId = $myTransactionId";
        $result = mysqli_query($conn,$sql);
        if ($result) {
            echo'<meta http-equiv="refresh" content="0;url=statusOrder.php">';
            mysqli_close($conn);
	    }
        else {
